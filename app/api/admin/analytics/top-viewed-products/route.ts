...
import { z } from "zod";
import { adminTopViewedProducts } from "@/services/personalization/analytics-admin";
...
export async function GET(req: Request) {
  try {
    ...
    const items = await adminTopViewedProducts(limit);
    return NextResponse.json({ ok: true, items });
  } catch (e) {
    console.error("[admin/analytics/top-viewed-products]", e);
