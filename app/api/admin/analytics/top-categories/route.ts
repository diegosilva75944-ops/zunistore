...
import { z } from "zod";
import { adminTopCategories } from "@/services/personalization/analytics-admin";
...
export async function GET(req: Request) {
  try {
    ...
    const items = await adminTopCategories(limit);
    return NextResponse.json({ ok: true, items });
  } catch (e) {
    console.error("[admin/analytics/top-categories]", e);
