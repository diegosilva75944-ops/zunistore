...
import { runTestMagaluImport } from "@/lib/magalu-test/pipeline";
import { extractMagaluProductIdFromUrl } from "@/lib/magalu-test/normalize";
import { buildNormalizedFromMagaluImport } from "@/services/magalu/pdp-import-mapper";
...
const schema = z.object({
  ...
  affiliateCode: z.string().min(1).optional().default("magalu_ext"),
});
...
export async function POST(req: Request) {
  try {
    ...
    if (!parsed.success) {
      return withCors(
        NextResponse.json(
          { ok: false, error: "Payload inválido: informe sourceUrl e affiliateUrl." },
          { status: 400 },
        ),
      );
    }
    ...
    const p = parsed.data;
    const fetchUrl = p.sourceUrl.trim();

    const externalId = extractMagaluProductIdFromUrl(fetchUrl);
    if (!externalId) {
      return withCors(
        NextResponse.json(
          { ok: false, error: "URL sem código de produto (/p/123…/)." },
          { status: 400 },
        ),
      );
    }

    const result = await runTestMagaluImport(fetchUrl, "auto");
    const normalized = buildNormalizedFromMagaluImport(result, externalId, fetchUrl);
    ...
    console.error("[import/magazinevoce]", e);
    return withCors(
      NextResponse.json(
