...
export async function generateMetadata(): Promise<Metadata> {
  const { total } = await listProducts({ perPage: 10, page: 1, sort: "maior-desconto", offersOnly: true });
  return {
...
export default async function MaioresDescontosPage() {
  const { items, total } = await listProducts({ sort: "maior-desconto", perPage: 20, page: 1, offersOnly: true });
