...
async function importProduct(baseUrl, token, payload) {
  const url = baseUrl.replace(/\/+$/, "") + "/api/admin/import/magazinevoce";
...
function isValidMagaluProductUrl(url) {
  try {
    const u = new URL(url);
    const hostOk =
      u.hostname.endsWith("magazinevoce.com.br") || u.hostname.endsWith("magazineluiza.com.br");
    return hostOk && /\/p\/\d+/i.test(u.pathname);
  } catch {
    return false;
  }
}
...
async function initPopup() {
  ...
  if (tab?.url && isValidMagaluProductUrl(tab.url)) {
...
async function main() {
  ...
  $("importBtn").addEventListener("click", async () => {
    ...
    if (!productUrl) {
      return setStatus("Abra uma página de produto Magalu ou cole a URL (com /p/CÓDIGO/).", true);
    }
    if (!affiliateUrl) return setStatus("Informe o link de afiliado (botão Comprar).", true);

    if (!isValidMagaluProductUrl(productUrl)) {
      return setStatus(
        "A URL deve ser Magazine Você ou Magalu e conter /p/ seguido do código numérico do produto.",
        true,
      );
    }
    ...
    try {
      ...
      setStatus("Importando no servidor…");

      const payload = {
        sourceUrl: productUrl,
        affiliateUrl,
        affiliateCode: "magalu_ext",
      };
