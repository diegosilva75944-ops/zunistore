...
function parseAndesMoney(el: Element) {
...
(async () => {
  ...
  try {
    ...
    if (await btn.isVisible().catch(() => false)) await btn.click({ timeout: 2000 });
  } catch {}

  await page.waitForTimeout(2000);

  const data = await page.evaluate(() => {
    const isInsideOtherSellers = (el: Element) => {
      try {
    ...
    const priceFromRow = (() => {
      ...
      return { amounts, price, promoLine, promoPrice };
    })();
    ...
    const priceFromMain = (() => {
      ...
      return { amounts, line1, promoLine, promoPrice };
    })();

    return { priceFromRow, priceFromMain };
  });
